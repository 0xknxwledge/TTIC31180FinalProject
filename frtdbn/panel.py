...
def build_lagged_design(
    ...
    min_periods: int = 60,
) -> tuple[np.ndarray, list[np.ndarray], list[str], pd.DatetimeIndex]:
    """Return the time-ordered (target, lags, column_names, target_timestamps).

    Returns -> rolling past-only z-score (warmup dropped) -> lagged design.
    """
...
def assemble_regime_design(
    ...
    target, lags, names, ts_target = build_lagged_design(
        frames_by_symbol, panel_symbols, p, zscore_window, min_periods
    )
